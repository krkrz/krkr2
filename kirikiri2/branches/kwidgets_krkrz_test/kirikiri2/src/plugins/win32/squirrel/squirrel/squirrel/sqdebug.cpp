...
void SQVM::Raise_ErrorF(const SQChar *s, ...)
{
	va_list vl;
	va_start(vl, s);
	Raise_ErrorV(s, vl);
...
void SQVM::Raise_IdxError(SQObject &o)
{
	...
	Raise_ErrorF(_SC("the index '%.50s' does not exist"), _stringval(oval));
}

void SQVM::Raise_CompareError(const SQObject &o1, const SQObject &o2)
{
	SQObjectPtr oval1 = PrintObjVal(o1), oval2 = PrintObjVal(o2);
	Raise_ErrorF(_SC("comparsion between '%.50s' and '%.50s'"), _stringval(oval1), _stringval(oval2));
}
...
void SQVM::Raise_ParamTypeError(SQInteger nparam,SQInteger typemask,SQInteger type)
{
	...
	Raise_ErrorF(_SC("parameter %d has an invalid type '%s' ; expected: '%s'"), nparam, IdType2Name((SQObjectType)type), _stringval(exptypes));
}
