	...
	for(i=0; i<nummixers; i++) // for each mixer
	{
		...
		for(j=0; j<(tjs_int)mxcaps.cDestinations; j++) // for each destination
		{
			...
			for(k=0; k<numcon; k++) // for each source
			{
				...
				{
					// the line is source
					DWORD componenttype = mxl.dwComponentType;
	...
	if(Drive != drv)
	{
		...
		if(err == 0)
		{
			...
			if(err)
			{
				...
				mciSendCommand(DeviceID, MCI_CLOSE, MCI_WAIT, (DWORD)&params);
			}
			else
			{
				...
				err = mciSendCommand(DeviceID, MCI_STATUS,
					 MCI_STATUS_ITEM | MCI_WAIT, (DWORD)&statusparams);
...
MCIERROR tTJSNI_CDDASoundBuffer::StartPlay()
{
	...
	_SetVolume(Volume);
	err = mciSendCommand(DeviceID, MCI_PLAY, flags , (DWORD)&playparams);
	return err;
}
//---------------------------------------------------------------------------
MCIERROR tTJSNI_CDDASoundBuffer::StopPlay()
{
	...
	if(Drive!=-1 && Status != ssUnload)
	{
		...
		MCIERROR err;
		err = mciSendCommand(DeviceID, MCI_STOP, MCI_WAIT, (DWORD)&genparams);
		return err;
	}
	return 0;
}
...
void tTJSNI_CDDASoundBuffer::_SetVolume(tjs_int v)
{
	...
	if(err == 0)
	{
		...
		mciSendCommand(params.wDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD)&genparams);
	}
