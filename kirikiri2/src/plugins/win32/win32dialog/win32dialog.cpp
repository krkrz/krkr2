...
public:
	...
	LRESULT callback(tjs_char const *event, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		// 引数を生成してコールバックを呼ぶ
		rslt.Clear();
		tTJSVariant vmsg((tjs_int)msg), vwp((tjs_int)wparam), vlp((tjs_int)lparam);
...
protected:
	...
	void _open(VarT win) {
		...
		if (DialogBoxIndirectParam(GetModuleHandle(0), (LPCDLGTEMPLATEW)ref, hwnd, (DLGPROC)DlgProc, (LPARAM)this) == -1)
			ThrowLastError();
	}
