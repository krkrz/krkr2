...
　get/set/call が実装されています。
  ...
  オブジェクトの参照は、squirrel の UserData としての参照になります。
　取得したオブジェクトを継承したり、Object クラスの委譲の対象
  として使うことはできません。

・Scripts.regist() で独自の登録を追加できます

・Scripts.registClass で、TJS2 のクラスを、Squirrel のクラスとして
　登録できます。この登録によってつくられたクラスは継承可能で、
　生成されたインスタンスを Object クラスの委譲対象にすることができます。
