...
オブジェクトをSquirrel の記述形式に変換して文字列か、
またはそれをファイルに保存できます。

------------------------------------------------------------------------------
var str = Scripts.toSQString( オブジェクト); // 文字列化
Scripts.saveSQ(ファイル名, オブジェクト);    // ファイル保存
------------------------------------------------------------------------------

文字列化の場合は純粋に式だけが抽出された形になります。
ファイル保存の場合は、return がついた以下の形式になります。
--------------------
return 式
--------------------

このファイルを execStorageSQ() 、または、squirrel の
スクリプト中から dofile() することで元のデータを抽出することができます。
